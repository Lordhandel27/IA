...
def construir_diccionario(tokens):
    ...
    diccionario[tokens[-1]] = []  # última palabra sin siguiente
    return diccionario
...
dic = construir_diccionario(tokens)
#for k, v in dic.items():
    #print(f"{k} -> {v}")

    # Ejemplo de uso
texto = "Las montañas se alzaban majestuosas bajo el cielo grisáceo, cubiertas por un manto de nubes pesadas que amenazaban con descargar una tormenta inminente. El viento susurraba entre los pinos, como si contara secretos antiguos de épocas olvidadas. En la pequeña cabaña al pie de la colina, la luz titilaba débil, reflejando la lucha entre la oscuridad que se imponía y el calor que aún se resistía a abandonarla. Marta miró hacia la ventana, sintiendo un escalofrío que no provenía únicamente del frío, sino de la incertidumbre que le envolvía el alma en aquellos días."
...
texto_generado = generar_texto(diccionario, palabra_inicial, 20)
print("\nTexto generado:\n", texto_generado)